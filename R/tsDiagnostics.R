...
#' @return A combined plot of geom_line_plot, ACF and PACF using patchwork and KPSS test of stationary result
#' @export
#'
...
tsDiagnostics <- function(y, x=NULL, lags=20) {
  ...
  test <- kpss.enhanced.test(y)
  if (test$statistic < test$critical_values[[3]]) cat(deparse(substitute(y)), "is Stationary") else cat(deparse(substitute(y)), "is not Stationary")
  cat("\n----------------------------------------------------------------\n")
